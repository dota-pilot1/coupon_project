...
import {
  ...
  useCreateTaskFolderMutation,
  useRenameTaskFolderMutation,
  useDeleteTaskFolderMutation
} from '@/entities/tasks/api/queries'
...
export default function TasksPage() {
  ...
  const createFolderMutation = useCreateTaskFolderMutation((parentId) => {
    setInlineFolderInput(null)
    setInlineFolderName('')
    if (parentId !== null) setExpandedFolders((p) => new Set(p).add(parentId))
  })

  const renameFolderMutation = useRenameTaskFolderMutation(() => {
  ...
  const handleDelete = async () => {
    ...
    const ok = await confirm({ title: '삭제 확인', description: '이 Task를 삭제하시겠습니까?' })
    if (ok) deleteMutation.mutate(selectedPostId)
  }

  const handleDeleteFolder = async (id: number, name: string) => {
    const ok = await confirm({ title: '폴더 삭제', description: `"${name}" 폴더와 하위 Task가 모두 삭제됩니다.` })
  ...
  const openCtxMenu = (e: React.MouseEvent, folder: TaskFolder) => {
    e.preventDefault()
    e.stopPropagation()
            ...
            <tr>
              ...
              <td className={tdStyle}>
                <input type="text" value={formTitle} onChange={(e) => setFormTitle(e.target.value)}
                  className="w-full border rounded px-2 py-1.5" placeholder="Task 제목" />
              </td>
            </tr>
          </tbody>
        </table>
  ...
  const renderDetail = (post: TaskPost) => {
    return (
      ...
      <div className="flex items-stretch" style={{ minHeight: 'calc(100vh - 120px)' }}>
        ...
        >
          <div className="flex items-center justify-between p-3 border-b bg-gray-50">
            <span className="font-medium text-sm">팀원</span>
            <button
          ...
          <div className="overflow-y-auto py-1" style={{ maxHeight: 'calc(100vh - 200px)' }}>
            {roots.length === 0 && !inlineFolderInput
              ? <p className="text-xs text-gray-400 text-center py-4">+ 팀원 버튼으로 추가하세요.</p>
              : roots.map((f) => renderFolder(f))}
            {inlineFolderInput?.parentId === null && renderInlineFolderInput(0)}
          </div>

          <div className="border-t p-2">
            <p className="text-[10px] text-gray-400 text-center">우클릭 또는 ··· 으로 Task 추가</p>
          </div>
        </div>
          ...
          {selectedFolderId && (
            <div className="bg-white rounded border">
              ...
              <div className="divide-y" style={{ maxHeight: '140px', overflowY: 'auto' }}>
                {posts.length === 0 ? (
                  <p className="text-sm text-gray-400 text-center py-4">우클릭 → 새 Task 추가</p>
                ) : (
                  posts.map((post) => {
                    const primaryType = post.blocks?.[0]?.blockType ?? (post as any).contentType ?? 'NOTE'
                    const meta = TYPE_META[primaryType as ContentType] ?? TYPE_META.NOTE
                ...
                : postDetail ? renderDetail(postDetail)
                  : (
                    <div className="flex items-center justify-center h-[300px] text-gray-400 text-sm">
                      {selectedFolderId ? '우클릭 → 새 Task 추가' : '좌측에서 팀원을 선택하세요.'}
                    </div>
                  )}
            </div>
          </div>
